                    ...
                    <TableBody>
                        {orders?.map((order) => (
                            <TableRow key={order.id}>
                                ...
                                <TableCell component="th" scope="row">{currentTRY.format(order.subTotal)}</TableCell>
                                <TableCell component="th" scope="row" sx={{ width: 100 }}>
                                    <Button onClick={() => handleDialogOpen(order)} size="small" variant="contained" endIcon={<ArrowRight />}>Details</Button>
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
            ...
            <Dialog onClose={handleDialogClose} open={open} fullWidth maxWidth="lg" aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                ...
                <DialogContent dividers>
                    <Paper sx={{ p: 3, mb: 3 }}>
                        ...
                        <Typography variant="subtitle2" gutterBottom>{selectedOrder?.addresLine} / {selectedOrder?.city}</Typography>
                    </Paper>
                    <TableContainer component={Paper}>
                        <Table>
                            ...
                            <TableBody>
                                {selectedOrder?.orderItems.map((item) => (
                                    <TableRow key={item.id}>
                                        <TableCell>
                                            <img src={`http://localhost:5273/images/${item.productImage}`} style={{ height: 60 }} />
                                        </TableCell>
                                ...
                                <TableRow>
                                    ...
                                    <TableCell align="right" colSpan={4}>{currentTRY.format(subTotal)}</TableCell>
                                </TableRow>
                                <TableRow>
                                    <TableCell align="right" colSpan={4}>Vergi</TableCell>
                                    <TableCell align="right" colSpan={4}>{currentTRY.format(tax)}</TableCell>
                                </TableRow>
                                <TableRow>
                                    <TableCell align="right" colSpan={4}>Toplam</TableCell>
                                    <TableCell align="right" colSpan={4}>{currentTRY.format(total)}</TableCell>
                                </TableRow>
                            </TableBody>
                        </Table>
                    </TableContainer>
                ...
                <DialogActions>
                    ...
                    <Button autoFocus onClick={handleDialogClose}>Kapat</Button>
                </DialogActions>
