            ...
            <Paper sx={{ marginTop: 8, padding: 2 }} elevation={3}>
                ...
                <Box component="form" onSubmit={handleSubmit(SubmitForm)} noValidate sx={{ mt: 2 }}>
                    <TextField
                        ...
                        label="Enter username" fullWidth autoFocus sx={{ mb: 2 }} size="small" error={!!errors.username} helperText={errors.username?.message}>
                    </TextField>

                    <TextField
                        {...register("name", { required: "name is required" })}
                        label="Enter name" fullWidth sx={{ mb: 2 }} size="small" error={!!errors.name} helperText={errors.name?.message}>
                    </TextField>
                    ...
                    <TextField
                        ...
                        label="Enter email" fullWidth sx={{ mb: 2 }} size="small" error={!!errors.email} helperText={errors.email?.message}>
                    </TextField>

                    <TextField
                        {...register("password", { required: "password is required", minLength: { value: 6, message: "min length is 6 charecters" } })}
                        label="Enter password" type="password" fullWidth sx={{ mb: 2 }} size="small" error={!!errors.password} helperText={errors.password?.message}>
                    </TextField>
                    <Button type="submit" loading={isSubmitting} disabled={!isValid} variant="contained" fullWidth sx={{ mt: 1 }}>Register</Button>
                </Box>
            </Paper>
        </Container>
